...
#define A_PM1P0_SAMPLE_BUFFER     (0)
#define A_PM2P5_SAMPLE_BUFFER     (1)
#define A_PM10P0_SAMPLE_BUFFER    (2)
#define B_PM1P0_SAMPLE_BUFFER     (3)
#define B_PM2P5_SAMPLE_BUFFER     (4)
#define B_PM10P0_SAMPLE_BUFFER    (5)
#define SO2_SAMPLE_BUFFER         (6)
...
#define EEPROM_SO2_BASELINE_VOLTAGE_TABLE (EEPROM_NTP_TZ_OFFSET_HRS - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
#define EEPROM_MQTT_TOPIC_SUFFIX_ENABLED  (EEPROM_SO2_BASELINE_VOLTAGE_TABLE - 1)    // a simple flag to indicate whether or not the topic suffix is enabled
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |    SO2 / PM Sensor Suite     |"));
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        eeprom_write_block("aqev2_so2_pm_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean add_baseline_voltage_characterization(char * arg, uint32_t eeprom_table_base_address) {
    ...
    if(!empty_location_found) {
        ...
        if(eeprom_table_base_address == EEPROM_SO2_BASELINE_VOLTAGE_TABLE) {
            Serial.print(F("so2"));
        }
...
boolean publishSO2() {
    clearTempBuffers();
    uint16_t num_samples = so2_ready ? sample_buffer_depth : sample_buffer_idx;
    float converted_value = 0.0f, compensated_value = 0.0f;
    float so2_moving_average = calculateAverage(&(sample_buffer[SO2_SAMPLE_BUFFER][0]), num_samples);
...
void printCsvDataLine() {
    ...
    num_samples = so2_ready ? sample_buffer_depth : sample_buffer_idx;
    if(init_so2_afe_ok && init_so2_adc_ok && (so2_ready || (sample_buffer_idx > 0))) {
        float converted_value = 0.0f, compensated_value = 0.0f;
        so2_moving_average = calculateAverage(&(sample_buffer[SO2_SAMPLE_BUFFER][0]), num_samples);
        so2_convert_from_volts_to_ppb(so2_moving_average, &converted_value, &compensated_value);
        so2_ppb = compensated_value;
